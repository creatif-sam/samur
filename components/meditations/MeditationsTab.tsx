...
export default function MeditationsTab() {
  ...
  const meditationTimeInsights = useMemo(() => {
    ...
    ownMeditations.forEach((m) => {
      ...
      hourCounts[hour] += 1
      if (hour >= 5 && hour < 12) slotCounts.Morning += 1
      else if (hour >= 12 && hour < 17) slotCounts.Afternoon += 1
      else if (hour >= 17 && hour < 22) slotCounts.Evening += 1
      else slotCounts.Night += 1
    })

    const preferredPeriod = periodCounts.morning >= periodCounts.evening ? 'Morning' : 'Evening'
    const preferredPeriodPct = Math.round(
      (Math.max(periodCounts.morning, periodCounts.evening) / ownMeditations.length) * 100
    )
    let bestHour = 0
    for (let hour = 1; hour < 24; hour++) {
      if (hourCounts[hour] > hourCounts[bestHour]) bestHour = hour
    }
    const commonTimeLabel = new Date(2000, 0, 1, bestHour, 0).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })
    const dominantSlot = Object.entries(slotCounts).sort((a, b) => b[1] - a[1])[0]?.[0] ?? 'Morning'

    return { total: ownMeditations.length, preferredPeriod, preferredPeriodPct, commonTimeLabel, dominantSlot }
  }, [meditations, userId])
    ...
    <div className="space-y-6 md:space-y-8 pb-10">
      ...
      <header className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="space-y-1 min-w-0">
            <h2 className="text-xl font-semibold tracking-tight">Meditations</h2>
            <p className="text-sm text-muted-foreground">Personal reflections and scripture insights</p>
          </div>
          <div className="flex items-center gap-2 justify-end flex-shrink-0">
            <Button size="icon" variant="ghost" onClick={() => setViewMode(viewMode === 'list' ? 'grid' : 'list')}>
              {viewMode === 'list' ? <LayoutGrid className="h-4 w-4" /> : <List className="h-4 w-4" />}
            </Button>
            <Button size="sm" variant="outline" onClick={() => setEditing({} as MeditationDB)}>
              Write meditation
            </Button>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-2 w-full">
          ...
          <select
            ...
            onChange={(e) => setVisibilityFilter(e.target.value as 'all' | 'private' | 'shared')}
          >
      ...
      {userId && accountCreatedAt && (
        <div className="rounded-lg border bg-background p-4 w-full overflow-hidden">
          <MeditationStreakBoard meditations={meditations} ownerId={userId} accountCreatedAt={accountCreatedAt} />
        </div>
      )}
      ...
      {meditationTimeInsights && (
        <div className="rounded-lg border bg-background p-4 w-full">
          <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Meditation Rhythm</p>
          <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
            <div className="rounded-md bg-muted/40 px-3 py-2">
              <p className="text-[11px] text-muted-foreground">Most common time</p>
              <p className="font-semibold text-violet-700 dark:text-violet-300">{meditationTimeInsights.commonTimeLabel}</p>
            </div>
            <div className="rounded-md bg-muted/40 px-3 py-2">
            ...
            <div className="rounded-md bg-muted/40 px-3 py-2">
              ...
              <p className="font-semibold">{meditationTimeInsights.dominantSlot}</p>
            </div>
          </div>
      ...
      {editing && (
        <MeditationComposer
          ...
          onCreated={() => { setEditing(null); loadMeditations() }}
        />
      )}
      ...
      {viewMode === 'list' && (
        <section className="divide-y border rounded-md w-full overflow-hidden">
          {filteredMeditations.map((m) => {
            ...
            const createdTime = createdAt.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })
                  ...
                  <div className="flex items-start justify-between gap-3 w-full">
                    <div className="space-y-1 min-w-0 flex-1">
                      <Link href={`/protected/meditations/${m.id}`}>
                        <h2 className="font-medium group-hover:underline truncate">{m.title}</h2>
                      </Link>
                      <p className="text-sm text-muted-foreground line-clamp-2">{m.scripture}</p>
                    </div>
                    <div className="flex items-center gap-1 md:gap-2 flex-shrink-0">
                      {userId === m.author_id && (
                        <Button size="icon" variant="ghost" onClick={() => setEditing(m)}><Pencil className="h-4 w-4" /></Button>
                      )}
                      <Button size="icon" variant="ghost" onClick={() => copyMeditation(m)}><Copy className="h-4 w-4" /></Button>
                    </div>
                  </div>
      ...
      {viewMode === 'grid' && (
        <section className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full">
          {filteredMeditations.map((m) => {
            ...
            const periodLabel = m.period === 'morning' ? 'Morning' : 'Evening'
            return (
              <div key={m.id} className="rounded-lg border bg-background active:bg-muted/60 transition overflow-hidden">
                <Link href={`/protected/meditations/${m.id}`} className="block px-4 md:px-5 py-4 md:py-5 space-y-3">
                  <h2 className="text-base font-medium leading-snug line-clamp-2">{m.title}</h2>
                  <p className="text-sm text-muted-foreground line-clamp-3">{m.scripture}</p>
                </Link>
                <div className="border-t" />
                <div className="flex items-end justify-between gap-3 px-3 md:px-4 py-3">
                  <div className="flex flex-col gap-1 min-w-0">
                    <span className="text-xs text-muted-foreground truncate">{createdDate}</span>
                  ...
                  <div className="flex items-center gap-2 md:gap-3 flex-shrink-0">
                    {userId === m.author_id && (
                      <Button size="icon" variant="ghost" className="h-9 w-9" onClick={() => setEditing(m)}><Pencil className="h-4 w-4" /></Button>
                    )}
                    <Button size="icon" variant="ghost" className="h-9 w-9" onClick={() => copyMeditation(m)}><Copy className="h-4 w-4" /></Button>
                  </div>
                </div>
              </div>
            )
